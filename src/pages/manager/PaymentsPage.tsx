...
export default function PaymentsPageManager() {
  ...
  const handleCreatePayment = async (e: React.FormEvent) => {
    e.preventDefault();
    try { 
      await createPayment(paymentForm).unwrap(); 
      setShowPaymentForm(false); 
    } catch (err) { console.error(err); }
  };
  const handleUpdatePayment = async (e: React.FormEvent) => {
    e.preventDefault(); if (!selectedPayment) return; try { await updatePaymentStatus({ id: selectedPayment.id, update: updateForm }).unwrap(); setShowUpdateForm(false); setSelectedPayment(null); } catch (err) { console.error(err); } };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50 p-4 lg:p-6 xl:p-8" style={{ paddingTop:'100px'}}>
                  ...
                  <div className="mt-4 flex gap-2">
                    <button
                      onClick={()=>{
                        ...
                        setUpdateForm({ status_id: p.status, months_paid:[p.payment_for_month], payment_method: p.payment_method||'', reference_number:p.reference_number||'', notes:p.notes||'' });
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-white/60">
                  <tr className="text-[11px] uppercase tracking-wide text-gray-500">
                    <Th>Tenant</Th><Th>Amount</Th><Th>Period</Th><Th>Due</Th><Th>Status</Th><Th>Method</Th><Th>Actions</Th>
                  </tr>
                </thead>
                      ...
                      <tr key={p.id} className="hover:bg-white/60">
                        ...
                        <Td>
                          <div className="flex items-center gap-2">
                            <button onClick={()=>{ setSelectedPayment(p); setUpdateForm({ status_id: p.status, months_paid: [p.payment_for_month], payment_method: p.payment_method || '', reference_number: p.reference_number || '', notes: p.notes || '' }); setShowUpdateForm(true); }} className="px-3 py-1.5 rounded-lg bg-blue-600/10 text-blue-600 text-[11px] font-medium hover:bg-blue-600/20 flex items-center gap-1">
                              <span className="material-icons text-[14px]">edit</span>Edit
                            </button>
      ...
      {showUpdateForm && selectedPayment && (
        <Modal onClose={()=>setShowUpdateForm(false)} title="Update Payment" icon="edit">
